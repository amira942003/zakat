...
import { useLanguage } from "@/Components/LanguageProvider"; // <-- import t()

export const Ma7acil = () => {
  const api = useApi();
  const { t } = useLanguage(); // <-- récupérer la fonction de traduction
  ...
  const saveZakatHistory = async () => {
    ...
    for (const crop of cropsWithZakat) {
      const zakatData = {
        ...
        corp_type: crop.cropType || t("form", "unknownCrop"),
      };
  ...
  const updateCrop = (index, field, value) => { const updated = [...crops]; updated[index][field] = value; setCrops(updated); };
  const removeCrop = (index) => { setCrops(crops.filter((_, i) => i !== index)); const newCollapsed = { ...collapsedCrops }; delete newCollapsed[index]; setCollapsedCrops(newCollapsed); };
  ...
  const getWateringMethodLabel = (method) => {
    const labels = {
      rain: t("form", "wateringOptions.rain"),
      mixed: t("form", "wateringOptions.mixed"),
      artificial: t("form", "wateringOptions.artificial")
    };
              ...
              <div className="form-header bg-gradient-to-r from-gray-50 to-emerald-50 border-b border-gray-200 p-4 md:p-6">
                <div className="flex items-start gap-3 md:gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-gray-600 text-xs md:text-sm break-words">{t("form", "nisab")}: {NISAB.toLocaleString()} كغ • {t("form", "addCropInfo")}</p>
                  </div>
                </div>
              </div>
                    ...
                    <div key={index} className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-lg overflow-hidden">
                      ...
                      >
                        <div className="flex items-center min-w-0 gap-2 md:gap-3">
                          ...
                          <h3 className="text-sm md:text-lg font-bold text-emerald-800 select-none truncate">
                            {crop.cropType || `${t("form", "crop")} ${index + 1}`}
                          </h3>
                          {crop.zakatDue > 0 && (
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          ...
                          <span className="text-xs md:text-sm text-emerald-600 font-medium hidden sm:inline">
                            {collapsedCrops[index] ? t("form", "show") : t("form", "hide")}
                          </span>
                        </div>
                      </div>
                      ...
                      <div 
                        className={`p-0 m-0 transition-all duration-300 ease-in-out overflow-hidden ${
                          collapsedCrops[index] ? "max-h-0 opacity-0" : "max-h-[3000px] opacity-100"
                        }`}
                      >
                        <div className="p-3 md:p-4 space-y-3 md:space-y-4">
                          ...
                          <div className="cal-input-bg">
                            ...
                            <div className="relative">
                              <input
                                ...
                                placeholder={t("form", "cropTypePlaceholder")}
                              />
                          ...
                          {/* ... tu peux continuer de la même façon pour les autres labels, select et placeholders ... */}
                          {/* Boutons */}
            ...
            <div className="mt-6 md:mt-8 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-lg p-4 md:p-6">
              <div className="flex gap-3 md:gap-4">
                <div className="bg-amber-100 p-2 md:p-3 rounded-full flex-shrink-0">
                  <span className="text-xl md:text-2xl"><WarninIcon /></span>
                </div>
                <div className="min-w-0">
                  <h3 className="font-bold text-amber-800 mb-1 text-sm md:text-base">{t("infoCard", "title")}</h3>
                  <p className="text-amber-700 text-xs md:text-sm break-words">{t("infoCard", "text", { nisab: NISAB })}</p>
                </div>
              </div>
            </div>
          </div>
