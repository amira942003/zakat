...
export const UserInfos = () => {
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      try {
        ...
        if (err) throw new Error(translate(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err || status !== 200) {
        setPopup({ message: data?.old_password || Object.values(data || {})[0] || translate("حدث خطأ", "Une erreur est survenue", "An error occurred"), type: "error" });
        setIsLoading(false);
        return;
      }
